...
```{r Correlation between density and quality with 95th percentile, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=quality,y=density),data=wine)+geom_jitter(alpha=0.2)+
ggtitle(label = "Scatterplot between density and quality with 95th percentile values")+
coord_cartesian(ylim = c(min(wine$density),quantile(wine$density,probs = 0.95)))
```

Above Scatterpot show a negative correlation between Density and quality,but its not so prominent.

```{r Correlation between alcohol and quality with 95th percentile, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=quality,y=alcohol),data=wine)+geom_jitter(alpha=0.2)+
ggtitle(label = "Scatterplot between alcohol and quality with 95th percentile values")+
coord_cartesian(ylim = c(min(wine$alcohol),quantile(wine$alcohol,probs = 0.95)))
```

From above plot we can say there is a small positive correlaiton between alcohol and quality.

# Bivariate Analysis

### Talk about some of the relationships you observed in this part of the investigation. How did the feature(s) of interest vary with other features in the dataset?
* There is a small negative correlation between density of the wine and quality of the wine,May be because of the strong positive correlation between residual.sugar and density, wines with high density may tend to  be sweeter,which reduces the taste of the wine.
...
There is negative correlation between density and alcohol content, 
