...
ggplot(data=wine) + geom_histogram(aes(x=fixed.acidity), fill="blue")+
  geom_histogram(aes(x=volatile.acidity), fill="deeppink")+
...
```{r boxplot of density with quality, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=quality,y=density),data=wine)+geom_boxplot(outlier.colour="red")+
ggtitle(label = "boxplot of density with quality")
```

Above Scatterpot show a negative correlation between Density and quality,but its not so prominent.

```{r boxplot of alcohol with quality, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=quality,y=alcohol),data=wine)+geom_boxplot(outlier.colour="red")+
ggtitle(label = "boxplot of alcohol with quality")
```
...
```{r Correlation between density and residual sugar full data with alcohol division, echo=FALSE, message=FALSE, warning=FALSE}
...
Above plot clearly shows  three distinguished alcohol levels with respect to density and residual sugar.
```{r Correlation between density and residual sugar full data with alcohol division, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=alcohol,y=density,color=quality.group),data=wine)+
  geom_point(alpha=0.5)+
  scale_color_brewer(type='qual',palette = 2)+
  ggtitle(label = "Scatterplot between density and resiudal sugar with alcohol buckets")+
  coord_cartesian(xlim=c(0,quantile(wine$residual.sugar,probs = 0.95)),ylim = c(min(wine$density),quantile(wine$density,probs = 0.95)))+facet_wrap(~quality.group)
```

```{r Correlation between chlorides and total sulphur dioxide data with quality division, echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot(aes(x=chlorides,y=total.sulfur.dioxide,color=(quality.group)),data=wine)+
  ...
  facet_wrap(~wine$quality.group)
```
Although a clear distinction between different quality groups can be observed for higher chloride levels,it difficult to distiquish in lower chlroride levels.
Facet wrap of different quality groups clearly shows the total sulfur dioxide vs chlorides distribution in low,mid and high quality wines.

```{r Correlation between chlorides and total sulphur dioxide data with quality division, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=quality,y=density,color=(cut(residual.sugar,breaks = c(min(wine$residual.sugar),5.2,max(wine$residual.sugar))))),data=wine)+
...
```{r Correlation between alcohol and fixed acidity  with quality division, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=wine$free.sulfur.dioxide,y=wine$total.sulfur.dioxide,color=(cut(alcohol,2))),data=wine)+
  geom_point(alpha=0.5)+
